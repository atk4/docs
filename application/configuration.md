...
In line with our design goal of simplicity, Agile Toolkit uses a PHP array for storing configuration values. Here's how it works:
    ...
    or will this be overwritten when they upgrade?
    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
1. Early in the execution flow the application loads the file `config-default.php` and adds its settings to the `$config` property in your api object.
